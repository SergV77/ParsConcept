...
#     for k, v in dict(sorted(dict_noun.items(), key=lambda x: len(x[1]), reverse=False)).items():
#         noun_adj_dict[k] = mix_noun_adj(" ".join(v), key)
...
#             for token in sent:
#                 # print(token.head.text, ' - ', token.dep_, ' - ', token.pos_, ' - ', token.text)
#                 if token.dep_ == 'ROOT' and token.pos_ == 'NOUN': #token.dep_ == 'ROOT' and
#                     chunk = ''
...
#################################################################################################################
#     if key != root_count:
#         if new_dict[key] in new_dict[root_count]:
#             tree.create_node(key.title(), key, parent=root_count)
#             for k, v in value.items():
#                 # print(k, ' - ', v)
#                 tree.create_node(k.title(), k, parent=key)
#                 if len(value) > 1:
#                     for token in value:
#                         #     print(token)
#                         #
